        ...
        it('Should be minified correct', () => {
            expect(ATOMIC_GLOBAL).toBe(
                [
                    ...
                    'case"$bind":return (p,s,o)=>{const nI=qAll(root,s);if(!nI.length)return;const c=get(p);if(c===undefined){set(p,getIV(nI,p));notify(p);}else{setIV(nI,c);}const fn=()=>{set(p,getIV(qAll(root,s),p));notify(p);};const sync=v=>setIV(qAll(root,s),v);sync._isSync=true;(subs[p]??=[]).push(sync);for(const n of nI){n.addEventListener("input",fn);if(n.type==="checkbox"||n.type==="radio"||n.tagName==="SELECT")n.addEventListener("change",fn);}if(isFn(o)){o._last=clone(get(p));subs[p].push(o);}else if(isFn(o?.handler)){o.handler._last=clone(get(p));subs[p].push(o.handler);if(o.immediate===true)o.handler(o.handler._last);}};',
                    ...
                    'def("$tfutils",(()=>{',
                    'return (ext={})=>{',
                    'const obj=Object.create(null);const oD=(n,v)=>def(n,v,obj);',
        ...
        it('Should be minified correct in non-debug mode', () => {
            expect(ARC_GLOBAL(false)).toBe(
                [
                    ...
                    '}',
                    'const $=ATOMIC?w.$tfutils(extensions):{};',
                    ...
                    'oD("$publish",(t,v)=>w.$tfr.publish(t,v),n);',
                    'try{',
                    'const ext=fn(ATOMIC?{mod:n,data:w.$tfdr({},data||{}),$}:{mod:n,data});',
                    'if(ATOMIC&&Object.prototype.toString.call(ext)==="[object Object]")extensions[name]=ext;',
                    '}catch(err){w.$tflog.debug("[Atomic] Module Instantiation Error",err);}}},',
                    '});})(),w);})(window,document);',
                ].join(''),
            );
        });
