        ...
        it('Should be minified correct', () => {
            expect(ATOMIC_GLOBAL).toBe(
                [
                    ...
                    'const s=Object.create(null);',
                    'const kP="$tfs:";',
                    'const notify=(k,v)=>win.$tfr.publish("$store:"+k,v);',
                    'try{',
                    ...
                    'const notify=p=>{let c="";const n_p=p.split(".");for(let i=0;i<n_p.length;i++){c=i===0?n_p[0]:c+"."+n_p[i];pending.add(c);}win.$tfcr(root.$uid);};',
                    ...
                    'case"$bind":return (p,s,o)=>{const nI=win.$tfutils.queryAll(root,s);if(!nI.length)return;const c=get(p);if(c===undefined){set(p,getIV(nI,p));notify(p);}else{setIV(nI,c);}const fn=()=>{set(p,getIV(win.$tfutils.queryAll(root,s),p));notify(p);};const sync=v=>setIV(win.$tfutils.queryAll(root,s),v);sync._isSync=true;(subs[p]??=[]).push(sync);for(const n of nI){n.addEventListener("input",fn);if(n.type==="checkbox"||n.type==="radio"||n.tagName==="SELECT")n.addEventListener("change",fn);}if(isFn(o)){o._last=clone(get(p));subs[p].push(o);}else if(isFn(o?.handler)){o.handler._last=clone(get(p));subs[p].push(o.handler);if(o.immediate===true)o.handler(o.handler._last);}};',
                    ...
                    'oD("create",(()=>{const s=new Set(["svg","path","circle","rect","g","defs","text","use","line","polyline","polygon","ellipse","symbol","clipPath","linearGradient","radialGradient","filter","mask","pattern"]);return (t,o={})=>{const e=s.has(t)?document.createElementNS("http://www.w3.org/2000/svg",t):document.createElement(t);if(o.attrs)for(const k in o.attrs)e.setAttribute(k,o.attrs[k]);if(o.style)for(const k in o.style)e.style[k]=o.style[k];if(o.children)for(const c of o.children)e.appendChild(isStr(c)?document.createTextNode(c):c);return e;};})());',
