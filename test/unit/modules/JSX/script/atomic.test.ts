        ...
        it('Should be minified correct', () => {
            expect(ATOMIC_GLOBAL).toBe(
                [
                    ...
                    'const kP="$tfs:";',
                    'const pub=(k,v)=>w.$tfr.publish("$store:"+k,v);',
                    'const pubExp=(k,v)=>w.$tfr.publish("$store:"+k+":expired",v);',
                    'let gttl=null;',
                    'let t=null;',
                    'let scanning=false;',
                    'try{',
                    ...
                    '};',
                    'return Object.freeze({',
                    'get:k=>{if(!isStr(k)||!k)return undefined;const e=s[k];if(!e)return undefined;if(e.exp&&e.exp<=Date.now()){expire(k,e,true);return undefined;}return e.v;},',
                    'set:(k,v,o={})=>{if(!isStr(k)||!k)return;const e=s[k];const ttl=o.ttl?Date.now()+o.ttl:null;const persist=o.persist===true;if(e&&eq(e.v,v)&&e.exp===ttl&&e.persist===persist)return;if(e?.persist&&!persist)delPersist(k);s[k]={v,exp:ttl,persist};if(persist){try{localStorage.setItem(kP+k,JSON.stringify({v,e:ttl}));}catch{}}pub(k,v);if(ttl&&(!gttl||ttl<gttl)){gttl=ttl;if(t)clearTimeout(t);t=setTimeout(scan,Math.max(0,ttl-Date.now()));}},',
                    'del:k=>{if(!isStr(k)||!k)return;const e=s[k];if(!e)return;expire(k,e,false);},',
                    '});',
                    '})());',
                    /* Global clock map */
                    'def("$tfc",new Map());',
