...
describe('Modules - Cookies', () => {
    ...
    describe('set', () => {
        it('Sets a valid cookie', () => {
            ...
            expect(cookies.outgoing).toEqual(['token=abc123; Path=/; Secure; HttpOnly']);
        });
        ...
        it('Sets Expires and derives Max-Age from expires', () => {
            ...
            expect(cookies.outgoing).toEqual([`session=val; Expires=${expires.toUTCString()}; Max-Age=300; Secure`]);
        });
        ...
        it('Prefers expires over maxAge when both provided', () => {
            ...
            expect(cookies.outgoing).toEqual([`token=abc; Expires=${expires.toUTCString()}; Secure`]);
        });
        ...
        it('Adds Path, Domain, HttpOnly, SameSite options', () => {
            ...
            expect(cookies.outgoing).toEqual(['token=val; Path=/secure; Domain=example.com; Secure; HttpOnly; SameSite=Lax']);
        });
        ...
        it('Ignores invalid options passed', () => {
            for (const el of CONSTANTS.NOT_OBJECT) {
                ...
                expect(cookies.outgoing).toEqual(['token=val; Path=/secure; Domain=example.com; Secure']);
            }
        });
        ...
        it('Forces Secure when SameSite=None and secure=false explicitly', () => {
            ...
            expect(cookies.outgoing).toEqual(['unsafe=val; SameSite=None; Secure']);
        });
        ...
        it('Adds new cookie without overriding previous', () => {
            ...
            cookies.set('one', '1');
            cookies.set('two', '2');
            expect(cookies.outgoing).toEqual(['one=1; Secure', 'two=2; Secure']);
        });
    ...
    describe('del', () => {
        ...
        it('Does nothing if providing invalid val', () => {
            ...
            expect(cookies.outgoing).toEqual(['shouldRemove=yup; Secure']);
        });
        ...
        it('Gracefully re-deletes a cookie that was already deleted', () => {
            ...
            expect(cookies.all()).toEqual({});
            expect(cookies.outgoing).toEqual([`dup=; Expires=${addUTC(now, 0, 'seconds').toUTCString()}; Max-Age=0; Secure`]);
        });
    ...
    describe('sign', () => {
        ...
        it('Returns empty string when value is not a string or number', async () => {
            ...
            for (const el of [...CONSTANTS.NOT_STRING, ...CONSTANTS.NOT_NUMERIC]) {
        ...
        it('Supports different algorithms when signing', async () => {
            ...
            expect(signed512).toBe(
                'sha512data.059a7015a544ce9c6e040cfa49b0ef7d26638c60535fce5388546ba311c0d5a34708e966a5d9e243bc54312ca589afa9b97ca8d2130a0b32fbf11abb3598996d',
            );

            const signed384 = await cookies.sign('sha384data', SECRET, {algorithm: 'SHA-384'});

            expect(signed384).toBe(
                'sha384data.4b8b69328ac5ec9c681678fe772c1259329a532f9d6c0146f4871217176cd39feeb2e6c2b374f858eab2bd0cc39c55ca',
            );
    ...
    describe('verify', () => {
        ...
        describe('Mixed secret/algorithm array', () => {
            ...
            it('Supports mixed array of strings and {val, algo} objects', async () => {
                ...
                const spy = vi.spyOn(cookies as any, 'generateHMAC');
                const verified = await cookies.verify(signed512, [WRONG_SECRET, {val: SECRET_SHA512, algorithm: 'SHA-512'}], {
                    algorithm: 'SHA-256',
                });
    ...
    describe('sign + set + verify', () => {
        ...
        it('Signs and sets a cookie, then verifies from combined map', async () => {
            ...
            expect(cookies.outgoing).toEqual(['session=userId42.a17e1fa187e5bef2af6e7c2c455364dfcd46ad8f09523100474125567d58d5b3; Secure']);
