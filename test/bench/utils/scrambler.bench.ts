...
const withValuePatternsScrambler = createScrambler({
    checks: [
...
const allCombinedScrambler = createScrambler({
    checks: ['user.password', {global: 'token'}, {valuePattern: /\d{3}-\d{2}-\d{4}/}, {valuePattern: /[\w.-]+@[\w.-]+\.\w+/}],
});
